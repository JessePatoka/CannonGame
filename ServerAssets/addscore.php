<?php
        ...
        $xpos = mysql_real_escape_string($_GET['xpos'], $db);
        $ypos = mysql_real_escape_string($_GET['ypos'], $db);
        $hash = $_GET['hash'];

        $secretKey="1j2hh5fg9"; # Change this value to match the value stored in the client javascript below

        $real_hash = md5($name . $score . $xpos . $ypos . $secretKey);
        if($real_hash == $hash) {
            // Send variables for the MySQL database class.
            $query = "insert into scores values (NULL, '$name', '$score', '$xpos', '$ypos');";
